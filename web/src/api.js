...
export async function fetchFloodExtent(jobId, returnPeriods = "all") {
  const param = Array.isArray(returnPeriods) ? returnPeriods.join(",") : returnPeriods
  return _request(
    "GET",
    `/api/jobs/${encodeURIComponent(jobId)}/results/flood-extent?return_periods=${encodeURIComponent(param)}`
  )
}
